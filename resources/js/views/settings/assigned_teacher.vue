<template>
  <div class="row vs-con-loading__container" style="overflow: visible;" id="permission_loading">
    <vs-prompt id="send_invoice" title="Adding New " :active.sync="active">
      <div class="row">
        <div class="col-lg-12">
          <vs-checkbox size="12px" class="mr-1" v-model="AssignedTeacher.main">Main Teacher</vs-checkbox>
          <div class="custom_input">
            <label for class="vs-input--label" style="display:block;">Assigned Teacher</label>
            <el-select
              v-model="AssignedTeacher.employee_id"
              filterable
              default-first-option
              placeholder="Choose a teacher"
            >
              <el-option
                v-for="teacher in teachers"
                :key="teacher.id"
                :label="teacher.first_name+' '+teacher.last_name"
                :value="teacher.id"
              ></el-option>
            </el-select>
          </div>
          <div class="custom_input">
            <label for class="vs-input--label" style="display:block;">Academic Year</label>
            <el-select
              v-model="AssignedTeacher.year_id"
              @change="changer_year_()"
              filterable
              default-first-option
              placeholder="Choose a year"
            >
              <el-option v-for="year in years" :key="year.id" :label="year.title" :value="year.id"></el-option>
            </el-select>
          </div>
          <div class="custom_input">
            <label for class="vs-input--label" style="display:block;">Classroom</label>
            <el-select
              v-model="AssignedTeacher.classroom_id"
              filterable
              default-first-option
              placeholder="Choose a classroom"
            >
              <el-option
                v-for="classroom in AssignedTeacher.classrooms"
                :key="classroom.id"
                :label="classroom.grade"
                :value="classroom.id"
              ></el-option>
            </el-select>
          </div>
        </div>

        <div class="col-lg-12 flex-end">
          <vs-button color="primary" class="edit_btn" @click="save()" type="filled">Save</vs-button>
          <vs-button color="warning" class="edit_btn" type="border" @click="active=false">Cancel</vs-button>
        </div>
      </div>
    </vs-prompt>
    <vs-prompt id="send_invoice" title="Edit" :active.sync="editactive">
      <div class="row">
        <div class="col-lg-12">
          <vs-checkbox size="12px" class="mr-1" v-model="AssignedTeacher.main">Main Teacher</vs-checkbox>
          <div class="custom_input">
            <label for class="vs-input--label" style="display:block;">Assigned Teacher</label>
            <el-select
              v-model="AssignedTeacher.employee_id"
              filterable
              default-first-option
              placeholder="Choose a teacher"
            >
              <el-option
                v-for="teacher in teachers"
                :key="teacher.id"
                :label="teacher.first_name+' '+teacher.last_name"
                :value="teacher.id"
              ></el-option>
            </el-select>
          </div>
          <div class="custom_input">
            <label for class="vs-input--label" style="display:block;">Academic Year</label>
            <el-select
              v-model="AssignedTeacher.year_id"
              @change="changer_year_()"
              filterable
              default-first-option
              placeholder="Choose a year"
            >
              <el-option v-for="year in years" :key="year.id" :label="year.title" :value="year.id"></el-option>
            </el-select>
          </div>
          <div class="custom_input">
            <label for class="vs-input--label" style="display:block;">Classroom</label>
            <el-select
              v-model="AssignedTeacher.classroom_id"
              filterable
              default-first-option
              placeholder="Choose a classroom"
            >
              <el-option
                v-for="classroom in AssignedTeacher.classrooms"
                :key="classroom.id"
                :label="classroom.grade"
                :value="classroom.id"
              ></el-option>
            </el-select>
          </div>
        </div>

        <div class="col-lg-12 flex-end">
          <vs-button color="primary" class="edit_btn" @click="update()" type="filled">Save</vs-button>
          <vs-button
            color="warning"
            class="edit_btn"
            type="border"
            @click="editactive=false;AssignedTeacher={}"
          >Cancel</vs-button>
        </div>
      </div>
    </vs-prompt>
    <div class="col-lg-12 flex-between-center">
      <h4 class="header-text flex-align-center" style="margin-bottom:0.6rem !important;">
        <vs-button
          radius
          size="40px"
          color="primary"
          icon-pack="feather"
          icon="icon-tag"
          type="flat"
        ></vs-button>Assigned Teacher
      </h4>
      <div class="flex-align-center">
        <div class="custom_input" style="margin-top:0px !important;">
          <el-select
            v-model="year_id"
            @change="filter_year()"
            filterable
            default-first-option
            placeholder="Choose a classroom"
          >
            <el-option v-for="year in years" :key="year.id" :label="year.title" :value="year.id"></el-option>
          </el-select>
        </div>
        <vs-button
          color="primary"
          type="filled"
          class="add_new"
          icon-pack="feather"
          icon="icon-plus"
          @click="add_new()"
        >Add</vs-button>
      </div>
    </div>
    <vs-divider class="mt-0" color="rgb(var(--vs-gray2),0.5)" />

    <div class="col-lg-12 mt-4">
      <div class="default_permissions_content">
        <div style="overflow-x:auto; overflow-y:hidden;" class="vs-table--tbody">
          <table class="permisson_table" id="subject-table" cellspacing="10">
            <thead>
              <tr class="table_header">
                <th class="px-3 text-center py-2">No.</th>
                <th class="px-3 text-center py-2">Teacher Name</th>
                <th class="px-3 text-center py-2">Classroom</th>
                <th class="px-3 text-center py-2">Action</th>
              </tr>
            </thead>
            <tbody>
              <tr
                class="table_content"
                v-for="(AssignedTeacher,index) in AssignedTeachers"
                v-bind:key="index"
              >
                <td
                  style="border-top-left-radius: .5rem;border-bottom-left-radius: .5rem;"
                >{{parseInt(index)+1}}</td>
                <td>
                  {{AssignedTeacher.teacher.first_name+' '+AssignedTeacher.teacher.last_name}}
                  <vs-icon
                    icon="el-icon-star-on"
                    icon-pack="el-icon"
                    v-if="AssignedTeacher.main"
                    color="warning"
                  />
                </td>
                <td>{{AssignedTeacher.classroom.grade}}</td>
                <td
                  class="flex-center"
                  style="border-top-right-radius: .5rem;border-bottom-right-radius: .5rem;"
                >
                  <vs-button
                    color="dark"
                    type="flat"
                    icon-pack="feather"
                    class="mr-1"
                    @click="editAssignedTeacher(AssignedTeacher,index)"
                    icon="icon-edit"
                  ></vs-button>
                  <vs-button
                    color="dark"
                    type="flat"
                    @click="deleteAssignedTeacher(AssignedTeacher.id)"
                    icon-pack="feather"
                    icon="icon-trash"
                  ></vs-button>
                </td>
              </tr>
            </tbody>
          </table>
        </div>
      </div>
    </div>
  </div>
</template>

<script>
export default {
  props: {
    current_year_id: null
  },
  data() {
    return {
      editactive: false,
      active: false,
      classroom_id: 1,
      classroom: "Grade 1",
      AssignedTeachers: [],
      data: [{}],
      year_id: null,
      AssignedTeacher: {
        classrooms: {},
        year_id: null
      },
      classroom_data: [{}],
      classrooms: {},
      teachers: {},
      edit_index: 0,
      years: []
    };
  },
  created() {
    if (localStorage.getItem("current_year_id")) {
      this.year_id = parseInt(localStorage.getItem("current_year_id"));
      this.AssignedTeacher.year_id = this.year_id;
    }

    this.getYears();
    this.getTeachers();
  },
  mounted() {},
  methods: {
    deleteAssignedTeacher(id) {
      axios
        .delete(`web/removeAssignedTeacher/${id}`)
        .then(response => {
          this.getAssignedTeachers();
        })
        .catch(error => {});
    },
    add_new() {
      this.AssignedTeacher.year_id = this.year_id;
      this.active = true;
    },
    update() {
      this.editactive = false;
      this.$vs.loading({
        container: "#permission_loading",
        background: "rgba(var(--vs-theme),0.4)",
        color: "primary",
        type: "border",
        scale: 0.6
      });
      axios
        .post(`web/updateAssignedTeacher`, { data: this.AssignedTeacher })
        .then(response => {
          this.$message({
            message: "Successfully updated!",
            type: "success"
          });
          this.getAssignedTeachers();
          this.$vs.loading.close("#permission_loading > .con-vs-loading");
        })
        .catch(error => {
          this.$vs.loading.close("#permission_loading > .con-vs-loading");
          this.$message({
            message: "Oops! There is an error while updating.",
            type: "danger"
          });
        });
    },
    editAssignedTeacher(data, index) {
      this.AssignedTeacher = data;
      var id = parseInt(this.AssignedTeacher.classroom.academic_year_id);
      this.AssignedTeacher.year_id = id;
      this.edit_index = index;
      this.editactive = true;
    },
    save() {
      this.active = false;
      this.$vs.loading({
        container: "#permission_loading",
        background: "rgba(var(--vs-theme),0.4)",
        color: "primary",
        type: "border",
        scale: 0.6
      });
      axios
        .post("/web/createAssignedTeacher", {
          data: this.AssignedTeacher
        })
        .then(response => {
          this.$message({
            message: "Successfully added!",
            type: "success"
          });
          this.getAssignedTeachers();
          this.$vs.loading.close("#permission_loading > .con-vs-loading");
        })
        .catch(error => {
          this.$vs.loading.close("#permission_loading > .con-vs-loading");
          this.$message({
            message: "Oops! There is an error while adding.",
            type: "danger"
          });
        });
    },
    getTeachers() {
      axios.get(`/web/getTeachers`).then(response => {
        this.teachers = response.data;
      });
    },
    getClassrooms() {
      axios.get(`/web/classrooms`).then(response => {
        this.classroom_data = response.data;
        this.changer_year();
        this.changer_year_();
      });
    },
    getAssignedTeachers() {
      this.$vs.loading({
        container: "#permission_loading",
        background: "rgba(var(--vs-theme),0.4)",
        color: "primary",
        type: "border",
        scale: 0.6
      });
      axios
        .get(`web/AssignedTeachers`)
        .then(response => {
          this.data = response.data;
          this.filter_year();
          this.$vs.loading.close("#permission_loading > .con-vs-loading");
        })
        .catch(error => {
          this.$vs.loading.close("#permission_loading > .con-vs-loading");
        });
    },
    filter_year() {
      this.AssignedTeachers = this.data.filter(data => {
        return parseInt(data.classroom.academic_year_id) === this.year_id;
      });
      this.AssignedTeacher.year_id = this.year_id;
      this.changer_year_();
    },
    changer_year_() {
      this.AssignedTeacher.classrooms = this.classroom_data.filter(data => {
        return parseInt(data.academic_year_id) === this.AssignedTeacher.year_id;
      });
      console.log("this.AssignedTeacher: ", this.AssignedTeacher);
    },
    changer_year() {
      this.classrooms = this.classroom_data.filter(data => {
        return parseInt(data.academic_year_id) === this.year_id;
      });
    },
    getYears() {
      axios
        .get(`web/years`)
        .then(response => {
          this.years = response.data;
          this.getClassrooms();
          this.getAssignedTeachers();
        })
        .catch(error => {});
    }
  }
};
</script>

